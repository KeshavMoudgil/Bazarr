...
export const fetchCustomerData = createAsyncThunk(
  ...
  async () => {
    ...
    const response = await fetch("http://localhost:51730/api/customer/data", {
      headers: {
...
export const addToCart = createAsyncThunk(
  ...
  async (productId) => {
    ...
    const response = await fetch(
      `http://localhost:51730/api/customer/cart/${productId}`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
...
export const placeOrder = createAsyncThunk("customer/placeOrder", async () => {
  const token = localStorage.getItem("token");
  const response = await fetch("http://localhost:51730/api/customer/order", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
